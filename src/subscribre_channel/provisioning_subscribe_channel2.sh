...
export LOG_FILE="log/provisioning.log"
mkdir -p log
...
export OPERATION_CODE=5
export OPERATION_DESC="Subscribe channel"
...
export SUBSCRIBE_CHANNEL_TEMPLATE="templates/provisioning_request.xml"
...
# WAIT ENTER TO CONTINUE
echo "Next, we simulate the client MO message '${USER_MO_ACCEPT_TEXT}' to subscribe." 
...
# SIMULATE CLIENT MO
echo "SIMULATE CLIENT MO..." | tee -a "${LOG_FILE}"
curl -D - -X GET "${URL_DESTINATION_SEND_MO}" | tee -a "${LOG_FILE}"
echo | tee -a "${LOG_FILE}"
echo | tee -a "${LOG_FILE}"
